...
class App extends React.Component {
  ...
  render() {
    return (
      <div id="clock">
        <audio id="beep" src="http://commondatastorage.googleapis.com/codeskulptor-demos/riceracer_assets/music/start.ogg"></audio>
        <h1>25 + 5 Clock</h1>
        <div id="tags-container">
          <h2 id="break-label">Break Lenght<p><button  id="break-decrement" onClick={this.minusBreak}>-</button><span id="break-length">{this.state.break}</span><button  id="break-increment" onClick={this.plusBreak}>+</button></p></h2>
          <h2 id="session-label">Session Lenght<p><button  id="session-decrement" onClick={this.minusSession}>-</button><span id="session-length">{this.state.session}</span><button  id="session-increment" onClick={this.plusSession}>+</button></p></h2>
        </div>
        <div id="session-container">
          <h1 id="timer-label">{this.state.label}</h1>
          <h1 id="time-left">{this.state.minutes < 10 ? '0' + this.state.minutes : this.state.minutes}:{this.state.seconds < 10 ? '0' + this.state.seconds : this.state.seconds}</h1>
          <p><button id="start_stop" onClick={this.start}>Start</button><button id="reset" onClick={this.reset}>Reset</button></p>
        </div>
      </div>
    );
  }
